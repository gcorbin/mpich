...
def parse_tap_output(build_output):
    ...
    for line in build_output:
        m = re.match(r'^(not\s+)?ok\s+(\d*\s+)-\s+([^#]*)\s+#(.*)', line)
        if m:
...
if __name__ == '__main__':
    ...
    try:
        ...
        with open(args.build_output, 'r') as file:
            build_num_skipped, build_num_failed = parse_tap_output(file)

        build_num_built = filter_num_retained - build_num_skipped - build_num_failed
        ...
        if build_num_built != run_num_total:
            raise RuntimeError("Mismatch in reported number of tests:"
                "{:4d} tests were successfully built, "
