...
Exclusion filters can read from a yaml file (See testlist-exlude-patterns.yaml).
The yaml should specify a dictionary with {place:patterns}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--filter', '-f', nargs='*', default=list())
    ...
    for filter in args.filter:
        if os.path.isfile(filter):
            logger.info("Loading filter file '%s'", filter)
            with open(filter) as file:
                pat = yaml.load(file, Loader=yaml.SafeLoader)
                for k,v in pat.items():
                    if k in exclude_patterns:
                        exclude_patterns[k].extend(v)
                    else:
                        raise KeyError("Unexpected key "+k+" in file "+filter)
